...
va_with_pop2 <- mutate(va_with_pop, Dose_1_share = Dose_1 / DP05_0021E, 
...
Nova_total <- Nova_vaccines %>% 
  ...
  mutate(Dose_1_share = Total_Dose_1 / pop, Dose_2_share = Total_Dose_2 / pop,
         Dose_1_share_eld = Total_Dose_1 / eld_pop, Dose_2_share_eld = Total_Dose_2 / eld_pop)
...
us_raw <- fromJSON("https://covid.cdc.gov/covid-data-tracker/COVIDData/getAjaxData?id=vaccination_trends_data")
...
current_total <- current_total %>% 
  mutate(Dose_1_share = Admin_Dose_1_Cumulative/total_adult, 
